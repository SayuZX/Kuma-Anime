...
async function fetchHomePageAniwatch() {
  try {
    ...
    if (response.ok) {
      ...
      console.error(
        `Terjadi kesalahan saat mengambil data dari Aniwatch API: ${response.status}`
      );
      return [];
    }
  } catch (error) {
...
async function fetchHomePageConsumet() {
  ...
  try {
    ...
    console.error(`Terjadi kesalahan saat mengambil data dari Consumet API: ${error}`);
  }
...
async function fetchAnimeDetailsConsumet(id) {
  try {
    ...
    if (resp.ok) {
      ...
      console.error(`Gagal mengambil data: ${resp.status}`);
      return null;
    }
  } catch (error) {
    console.error(error);
...
async function fetchAnimeDetailsAniwatch(id) {
  try {
    ...
    console.error(`Terjadi kesalahan saat mengambil detail anime: ${error}`);
    return null;
  }
}
